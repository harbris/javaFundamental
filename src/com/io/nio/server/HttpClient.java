...
public class HttpClient {
	public static void main(String[] args) {
		...
		HttpClient hc = new HttpClient(host, port, requestLine, headerMap,
				bodyBytes);
		hc.send();
	}
	...
	public HttpClient(String host, int port, String requestLine,
		...
		if (bodyBytes != null)
			sb.append("Content-Length:").append(bodyBytes.length)
					.append("\r\n");
	...
	private static final ByteBuffer readBuffer = ByteBuffer
			.allocate(HttpClient.BUFFER_SIZE);
	...
	public void send() {
		...
		try {
			...
			while (!isEnd) {
				...
				Iterator<SelectionKey> selectionKeyIter = selectionKeys
						.iterator();
				while (selectionKeyIter.hasNext()) {
					...
					if (selectionKey.isReadable()) {
						...
						MessageBag messageBag = (MessageBag) selectionKey
								.attachment();
						if (messageBag == null) {
						...
						while (readBuffer.hasRemaining()) {
							...
							if (messageBag.status == Status.INIT) {
								...
								if (oneByte == LF) {
									BodyStyle bodyStyle = messageBag
											.afterHeader();
								...
								if (messageBag.bodyStyle == BodyStyle.CONTENT_LENGTH) {
									// Content-Length 헤더가 있다면 해당 헤더 값만큼 바디를 추가로
									// 읽습니다.
									messageBag.add(oneByte);
									if (messageBag.getContentLength() <= messageBag
											.getBytesSize()) {
										// 메시지 바디를 끝까지 다 읽었다면
										// 지금까지 읽은 값을 bodyBytes에 넣고, 상태를 종료로
										// 표시하고 나갑니다.
									...
									if (messageBag.chunkStatus == ChunkStatus.CHUNK_NUM) {
										...
										if (oneByte == LF) {
											...
											throw new IllegalStateException(
													"LF must be followed by CR");
										}
									} else if (messageBag.chunkStatus == ChunkStatus.CHUNK_BODY) {
										if (messageBag.getBytesSize() == messageBag
												.getChunkSize() - 1) {
											messageBag.add(oneByte);
											messageBag.addChunk();
